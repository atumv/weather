...
function Weather (props) {
  ...
  return (
    <div className="weather">
      ...
      {props.city && 
        <p className="weather-city">{props.city}, {props.country}</p>
      }

      {props.temperature &&
        <p className="weather-temp">{plusSign}{Math.round (props.temperature)}°C</p>
      }

      {props.description &&
        <div className="weather-description">
          <img className="weather-description-icon" src={props.icon} alt="icon" />
          <span className="weather-description-text">{props.description}</span>
        </div>
      }

      {props.windSpeed &&
        <p className="weather-wind">Ветер: {Math.round (props.windSpeed)} м/с {windDirection}</p>
      }
